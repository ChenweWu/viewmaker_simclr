...
from .simclr import SimCLRObjective
from .infonce import NoiseConstrastiveEstimation


class AdversarialSupConLoss(object):

    def __init__(self, temp: float, t=0.07, view_maker_loss_weight=1.0) -> Tensor:
        super().__init__()
        self.temp = temp
        self.t = t
        self.view_maker_loss_weight = view_maker_loss_weight
        self.supcon_loss_fn = SupConLoss(temp)
    ...
    def forward(self, zs: Tuple[Tensor, ...], lbls: Tensor) -> Tensor:
        encoder_loss = self.supcon_loss_fn(zs, lbls)
